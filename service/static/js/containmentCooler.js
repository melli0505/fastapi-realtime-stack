...
$(document).ready(() => {
  ...
  $.ajax({
    url: `/api/containment/water-temp-date?start=${
    ...
    success: (data) => {
      ...
      data.forEach((el) => {
        ...
        recent[0] = el["inner_temp"];
        tempChart.data.datasets[1].data.push({
          x: new Date(el["time"]),
          y: el["outer_temp"],
        });
        recent[1] = el["outer_temp"];
        console.log(el);
      });
      waterInChart.data.datasets[0].data = recent[0] != 0 ? [recent[0]] : [-1];
      waterOutChart.data.datasets[0].data = recent[1] != 0 ? [recent[1]] : [-1];
      tempChart.update();
      waterInChart.update();
      waterOutChart.update();
    },
  });
...
function setData() {
  ...
  if (start == "" || end == "") alert("Please select both of dates.");
    ...
    $.ajax({
      url: `/api/containment/water-temp-date?start=${start_day}&end=${end_day}`,
      method: "GET",
      success: (data) => {
        ...
        waterInChart.data.datasets[0].data =
          recent[0] != 0 ? [recent[0]] : [-1];
        waterOutChart.data.datasets[0].data =
...
var server_id = 1;
const waterInChartx = document.getElementById("waterIn");
const waterOutChartx = document.getElementById("waterOut");
const tempChartx = document.getElementById("temperature");

var waterInChart = new Chart(waterInChartx, {
  type: "doughnut",
  data: {
    labels: [
      // 'Temperature',
    ...
    datasets: [
      {
        label: "Recent",
        data: [],
        backgroundColor: ["rgb(173, 215, 255)", "rgb(243, 255, 227)"],
        hoverOffset: 4,
      },
    ],
  },
...
var waterOutChart = new Chart(waterOutChartx, {
  type: "doughnut",
  data: {
    labels: [
      // 'Temperature',
    ...
    datasets: [
      {
        label: "Recent",
        data: [],
        backgroundColor: ["rgb(100, 150, 205)", "rgb(255, 250, 229)"],
        hoverOffset: 4,
      },
    ],
  },
